...
int main(int argc, char** argv) {
  ...
  while (true) {
    ...
    wstring prefix = q.s2ws(QgramIndex::split(query));
    Recordlist res = q.findMatches(prefix, prefix.size()/4, 10, true);
    for (size_t i = 0; i < res.size(); ++i) {
      std::cout << QgramIndex::ws2s(res[i]._City)
      << " (" << res[i]._Country << ")" << std::endl;
    }
  }
  return 0;
}
